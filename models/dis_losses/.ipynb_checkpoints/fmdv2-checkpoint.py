...
from models.utils import MultiheadAttention, resize_pos_embed, to_2tuple
...
class FreqMaskingDistillLossv2(nn.Module):
    ...
    def get_spat_loss(self, preds_S, preds_T):
        ...
        N, C, H = preds_T.shape
        # N, N_P, P_E_D = preds_T.shape
        # H = W = int(np.sqrt(N_P))
        # C = P_E_D
        device = preds_S.device

        dct = DCT(resolution=H, resolution2=C, device=device)
...
class AttentionProjector(nn.Module):
    ...
    def forward(self, x, query=None):
        ...
        if x.shape[2] != self.hw_dims[0] and x.shape[3] != self.hw_dims[1]:
            # 使用差值操作
            x = F.interpolate(x, size=(self.hw_dims[0], self.hw_dims[1]), mode='bilinear', align_corners=False)
...
class DCT():
    def __init__(self, resolution, resolution2, device, norm=None, bias=False):
        ...
        I2 = torch.eye(self.resolution2, device=self.device)
        self.forward_transform2 = nn.Linear(resolution2, resolution2, bias=bias).to(self.device)
        self.forward_transform2.weight.data = self._dct(I2, norm=self.norm).data.t()
        self.forward_transform2.weight.requires_grad = False
        ...
        self.inverse_transform2 = nn.Linear(resolution2, resolution2, bias=bias).to(self.device)
        self.inverse_transform2.weight.data = self._idct(I2, norm=self.norm).data.t()
        self.inverse_transform2.weight.requires_grad = False
    ...
    def forward(self, x):
        ...
        X2 = self.forward_transform2(X1.transpose(-1, -2))
        return X2.transpose(-1, -2)

    def inverse(self, x):
        X1 = self.inverse_transform(x)
        X2 = self.inverse_transform2(X1.transpose(-1, -2))
        return X2.transpose(-1, -2)